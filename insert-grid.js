...
// Buscar el cierre de la tabla y el div del CTA que le sigue, sin depender del texto del botón
code = code.replace(
  /(<\/table>\s*<div className="mt-4 text-center"><Link href="\/register"[^>]*>.*?<\/Link><\/div>\s*)<\/div>/,
  '$1' + grid + '\n          </div>'
);

fs.writeFileSync('src/app/page.tsx', code);
console.log('Grid insertado.');
