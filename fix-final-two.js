...
// 1. FAQ: reemplazar array duplicado por 8 preguntas limpias
...
// 2. Eliminar segundo bloque "El mercado freelance está roto"
const mercadoText = '⚠️ El mercado freelance está roto';
let first = code.indexOf(mercadoText);
let second = code.indexOf(mercadoText, first + 1);
if (second > -1) {
  let sectionStart = code.lastIndexOf('<section', second);
  let sectionEnd = code.indexOf('</section>', second) + 10;
  if (sectionStart > -1 && sectionEnd > 10) {
    code = code.substring(0, sectionStart) + code.substring(sectionEnd);
    console.log('✅ Segundo bloque del mercado eliminado.');
  }
}

// 3. Cambiar texto de garantía
code = code.replace(
...
console.log('✅ FAQ, mercado y garantía corregidos.');
