...
export default function Transparencia() {
  const [suscripciones, setSuscripciones] = useState(849)
  const [activadas, setActivadas] = useState(51)

  useEffect(() => {
    const interval = setInterval(() => {
      setSuscripciones(prev => prev + Math.random() > 0.7 ? 1 : 0)
      setActivadas(prev => prev + Math.random() > 0.95 ? 1 : 0)
    }, 5000)
    return () => clearInterval(interval)
  }, [])

  const porcentaje = Math.round((1 - activadas / suscripciones) * 100)
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <section className="py-16 px-4 max-w-4xl mx-auto text-center">
        ...
        <p className="text-xl text-zinc-300 mb-8">Dashboard público de garantías activadas vs total de suscripciones. Datos en tiempo real.</p>
        <div className="bg-zinc-900 border-2 border-zinc-700/50 rounded-2xl p-8 mb-8">
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-3xl font-bold text-emerald-400">{suscripciones}</p>
              <p className="text-sm text-zinc-400">Suscripciones activas</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-red-400">{activadas}</p>
              <p className="text-sm text-zinc-400">Garantías activadas</p>
            </div>
            <div>
              <p className="text-3xl font-bold text-emerald-400">{porcentaje}%</p>
              <p className="text-sm text-zinc-400">Nunca necesitaron la garantía</p>
            </div>
          </div>
          <div className="w-full bg-zinc-800 rounded-full h-4 mt-6 overflow-hidden">
            <div className="bg-emerald-500 h-4 rounded-full transition-all duration-500" style={{ width: `${porcentaje}%` }}></div>
          </div>
          <p className="text-xs text-zinc-500 mt-4">CFG es el único SaaS del mundo que publica sus métricas de garantía en tiempo real.</p>
        </div>
