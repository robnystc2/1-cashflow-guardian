...
export default function VsHoneyBook() {
  const rows = [
    { feat: 'Protección anti-impago', hb: '✗', cfg: '✓ Garantía Blindaje Total' },
    { feat: 'Bloqueo de entrega', hb: '✗', cfg: '✓ Candado por hito' },
    { feat: 'Mercado objetivo', hb: 'EE.UU.', cfg: 'España y LatAm' },
    { feat: 'Soporte IVA/IRPF', hb: '✗', cfg: '✓ Integrado' },
    { feat: 'Precio', hb: '$19/mes (subió 40% en 2024)', cfg: '29€/mes fijo' },
  ]
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <section className="py-16 px-4 max-w-4xl mx-auto text-center">
        <h1 className="text-4xl md:text-6xl font-bold mb-4">CFG vs <span className="text-zinc-400">HoneyBook</span></h1>
        <p className="text-xl text-zinc-300 mb-10">HoneyBook es para el mercado americano. CFG está diseñado para autónomos españoles y latinoamericanos.</p>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="border-b border-zinc-800 text-sm"><th className="py-3 px-4">Funcionalidad</th><th className="py-3 px-4 text-center">HoneyBook</th><th className="py-3 px-4 text-center bg-emerald-900/20 text-emerald-400 font-semibold">CFG</th></tr></thead>
            <tbody className="text-sm">
              {rows.map((row, i) => (
                <tr key={i} className="border-b border-zinc-800 hover:bg-white/[0.02]">
                  <td className="py-3 px-4 text-zinc-300">{row.feat}</td>
                  <td className="py-3 px-4 text-center text-zinc-400">{row.hb}</td>
