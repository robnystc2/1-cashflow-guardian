...
export default function VsDubsado() {
  const rows = [
    { feat: 'Protección anti-impago', dubsado: '✗', cfg: '✓ Garantía Blindaje Total' },
    { feat: 'Bloqueo de entrega', dubsado: '✗', cfg: '✓ Candado por hito' },
    { feat: 'Curva de aprendizaje', dubsado: 'Semanas', cfg: '✓ 3 minutos' },
    { feat: 'Precio', dubsado: '20$/mes', cfg: '29€/mes (con Garantía)' },
  ]
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <section className="py-16 px-4 max-w-4xl mx-auto text-center">
        <h1 className="text-4xl md:text-6xl font-bold mb-4">CFG vs <span className="text-zinc-400">Dubsado</span></h1>
        <p className="text-xl text-zinc-300 mb-10">Dubsado tarda semanas en configurarse. CFG está listo en 3 minutos.</p>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead><tr className="border-b border-zinc-800 text-sm"><th className="py-3 px-4">Funcionalidad</th><th className="py-3 px-4 text-center">Dubsado</th><th className="py-3 px-4 text-center bg-emerald-900/20 text-emerald-400 font-semibold">CFG</th></tr></thead>
            <tbody className="text-sm">
              {rows.map((row, i) => (
                <tr key={i} className="border-b border-zinc-800 hover:bg-white/[0.02]">
                  <td className="py-3 px-4 text-zinc-300">{row.feat}</td>
                  <td className="py-3 px-4 text-center text-zinc-400">{row.dubsado}</td>
